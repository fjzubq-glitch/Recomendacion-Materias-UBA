// ==========================================================================
// update.js - Botón oculto + atajo para disparar la actualización de datos en
// GitHub Actions desde el frontend estático (GitHub Pages).
//
...
//   - O tocando el pequeño botón flotante escondido (abajo a la izquierda)
// ==========================================================================
...
async function runUpdate() {
    ...
    if (!token) {
        setStatus('Primero guardá un token con permiso "workflow".', '#f87171');
        return;
    }
    setStatus('Disparando actualización en GitHub Actions...', 'var(--text-secondary)');
    try {
        ...
        if (resp.ok) {
            setStatus('Actualización iniciada ✔ (tarda ~4-5 min).', '#34d399');
            showToast('Actualización de datos iniciada', 'success');
        } else {
            const txt = await resp.text().then(() => resp.status).catch(() => resp.status);
            setStatus(`Error ${resp.status}` + (resp.status === 401 ? ' — token sin permiso "workflow".' : resp.status === 404 ? ' — workflow no encontrado.' : ''), '#f87171');
            showToast('No se pudo iniciar la actualización (' + resp.status + ')', 'error');
        }
    } catch (e) {
    ...
    modal.innerHTML = `
        <div class="modal-container" style="max-width: 480px;">
            <div class="modal-header">
                <div class="modal-titles"><h2>🔄 Actualizar catálogo</h2></div>
            ...
            <div class="modal-body" style="padding:1.5rem; color:var(--text-secondary); line-height:1.5;">
                <p style="margin-bottom:1rem;">
                    ...
                    y publica la nueva versión del sitio (<b>tarda ~4-5 min</b>).
                </p>
                ...
                <p style="font-size:0.75rem; margin-bottom:1rem;">Para <em>no</em> perder el token por sesión,
                    guardalo con el botón de abajo. Requiere el scope <code>workflow</code>.</p>
                <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                    <button class="btn-primary btn-inverse" id="btn-save-token">Guardar token</button>
                    <button class="btn-primary" id="btn-run-update" style="background:linear-gradient(135deg,#6366f1,#4f46e5);">Ejecutar actualización</button>
                </div>
                ...
                <a href="https://github.com/${REPO}/actions/workflows/${WORKFLOW}" target="_blank" rel="noopener"
                   style="font-size:0.8rem; color:var(--accent-color);">Ver progreso en GitHub Actions →</a>
            </div>
        </div>
    `;
...
function buildTriggerButton() {
    ...
    b.title = 'Actualizar catálogo (Ctrl+Shift+A)';
    b.setAttribute('aria-label', 'Actualizar catálogo');
    b.style.cssText = `
    ...
    b.addEventListener('click', () => { buildModal(); modal.style.display = 'flex'; });
...
function onKey(e) {
    ...
    if (e.altKey && (k === 'U')) { e.preventDefault(); buildModal(); modal.style.display = 'flex'; }
    else if (k === 'U' || k === 'A') { e.preventDefault(); buildModal(); modal.style.display = 'flex'; }
}
